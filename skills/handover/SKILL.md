---
name: handover
description: Create a clean structured handover document for a fresh agent to continue the work. Use when the user types /handover or asks to hand over the current session. Saves only to the global ~/data/handovers directory and never touches project files.
argument-hint: Optional focus for the next session
disable-model-invocation: true
---

Create a one-time handover document so a fresh agent can continue the work cleanly.
...
- Do not create, update, or leave any pointer, reference, or mention of this handover file inside the project (no README notes, no links, no comments, no git commits about it).
- This is a disposable transfer document. It must never be treated as living project documentation.
- Do not update or expand any previous handover files. Always create a brand-new file.
...
> Created: <timestamp>
> This file is a one-time handover. Do not treat it as project documentation. Do not keep it updated. Do not reference it from the codebase.
...
- A one-sentence confirmation that the handover is ready
